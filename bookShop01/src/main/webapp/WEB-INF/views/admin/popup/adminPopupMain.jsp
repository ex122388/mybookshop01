...
<script>
	function search_goods_list(search_period) {
		...
		formObj.method = "get";
		formObj.action = "/bookshop01/admin/popup/adminPopupMain.do";
		formObj.submit();
	}
...
function fn_send_popup(){
	...
	var h_chk_state=document.frmGoods.h_chk_state; //최초화면 표시에 표시하는 
...
//	alert(_goods_id);
	...
	$.ajax({
		...
		url : "http://localhost:8091/bookshop01/admin/popup/addPopup.do",
		data : {
			goods_id:_goods_id,
	...
	for(var i=0;i<h_chk_state.length;i++){
		...
		_goods_id = h_chk_goods_id[i].value;
			...
			$.ajax({
				type : "post",
				async : false, //false인 경우 동기식으로 처리한다.
				url : "http://localhost:8091/bookshop01/admin/popup/deletePopup.do",
				data : {
					goods_id:_goods_id,
...
<body>
			...
			<tr>
				<td colspan=8 class="fixed"><c:forEach var="page" begin="1"
						...
						<c:if test="${chapter >1 && page==1 }">
							<a
								href="${pageContext.request.contextPath}/admin/goods/adminGoodsMain.do?chapter=${chapter-1}&pageNum=${(chapter-1)*10 +1 }">&nbsp;pre
								&nbsp;</a>
						</c:if>
						<a
							href="${pageContext.request.contextPath}/admin/goods/adminGoodsMain.do?chapter=${chapter}&pageNum=${page}">${(chapter-1)*10 +page }
						</a>
						<c:if test="${page ==10 }">
							<a
								href="${pageContext.request.contextPath}/admin/goods/adminGooodsMain.do?chapter=${chapter+1}&pageNum=${chapter*10+1}">&nbsp;
								next</a>
						</c:if>
					</c:forEach> <%--              <c:choose>
	...
	<table>
		...
		<tr>
			...
			<td><textarea cols="70" rows="5" id="t_popup_message">메세지를 입력해주세요
 </textarea></td>

		</tr>

		<tr align="center">
			<td colspan="2"><input type="button" value="팝업등록" onClick="fn_send_popup()"/>
